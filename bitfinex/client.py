...
class Public(BaseClient):
    ...
    def get_last(self, symbol='btcusd'):
        """shortcut for last trade"""
        return float(self.ticker(symbol)['last_price'])
        
    def lendbook(self, limit_bids=50, limit_asks=50, currency='btc'):
        """get the margin funding book"""
        data = {}
        data['limit_bids'] = str(limit_bids)
        data['limit_asks'] = str(limit_asks)
        return self._get("/v1/lendbook/"+currency, data=data, return_json=True)
...
class Trading(Public):
    ...
    def new_offer(self, currency='btc', amount=0.1, rate=3.0,
                  period=30, direction='lend'):
        """
        submit a new offer
        ...
        period (integer): Number of days of the funding contract
        ...
        return self._post("/v1/offer/new", data=data, return_json=True)

    def cancel_offer(self, offer_id):
        """
        cancel an offer with offer_id
        """
        data = {'offer_id': offer_id}
        return self._post("/v1/offer/cancel", data=data, return_json=True)

    def offers(self):
        """
        view your active offers
        """
        return self._post("/v1/offers", return_json=True)
